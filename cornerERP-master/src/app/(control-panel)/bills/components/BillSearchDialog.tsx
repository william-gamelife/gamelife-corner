...
function BillSearchDialog({ open, onClose, onSearch, initialParams }: BillSearchDialogProps) {
	...
	const handleSearch = () => {
		...
		const serializedParams: SerializedSearchParams = {
			...searchParams,
			dateFrom,
			dateTo
		};
	...
	const handleReset = () => {
		...
		// 將重設參數也序列化
		const serializedDefaultParams: SerializedSearchParams = {
			...defaultParams,
			dateFrom: undefined,
			dateTo: undefined
		};
				...
				>
					...
					>
						<TextField
							...
							onChange={(e) => setSearchParams({ ...searchParams, billNumber: e.target.value })}
						/>
					</Grid>
					<Grid
						item
...
export default BillSearchDialog;
